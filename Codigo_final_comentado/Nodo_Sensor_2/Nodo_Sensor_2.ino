...
/*****************************  Nodo_Sensor_1.ino  ****************************/
/*                                                                            */
/*   Purpose: Módulo de comunicacion maestro para sistema de monitoreo        */
/*                                                                            */
...
/*                                                                            */
/*   Pendientes: Comunicación bidireccional Bluetooth                        */                              
/*                                                                            */
...
#define tiempo_muestreo 911                 // Contador de tiempo usado para definir le periodo de envío de los datos promediados
/******************************************************************************/
...
byte direccion[5] ={'y','a','n','a','1'};  //Dirección Nodo sensor 2 
/******************************************************************************/

/******************************Máquina de estados******************************/
enum estados {LECTURA, ESPERA, ENVIO, ENVIO_OK, ENVIO_NO_OK, TIMER};      // Estado
enum estados ESTADO;                        //Estado actual
/******************************************************************************/
...
/***********************************módulo HX711*******************************/ 
  ...
  balanza.set_scale(-1112.05);              // Se establece la escala previamente obtenida de la calibración
  balanza.set_offset(-32409);               // Se establece el offset previamente obtenido de la calibración
  /***********************************módulo HX711*******************************/ 
  balanza2.begin(DOUT2, CLK2);              // inicializa el HX711
  balanza2.set_scale(1205.528409);          // Se establece la escala previamente obtenida de la calibración
  balanza2.set_offset(658093);              // Se establece el offset previamente obtenido de la calibración
...
/*-------------------------------Estado LECTURA: -----------------------------*/     
  case LECTURA:   
    ...
    datos[2] = (dht.readHumidity()*1.014-6.9045)*10;     // Se obtiene valor de humedad
    datos[3] = (dht.readTemperature()*0.9429+1.9029)*10; // Se obtiene valor de temperatura
    datos[4] = 0;
    radio.write(datos, sizeof(datos));       // Se envían los datos por RF
    if (Promedios_tomados < Promedio){  
      // Si no se han tomado, se agregan los datos reciben leídos a los vectores de promedios         
      ...
      ESTADO=ESPERA;    // Se actualiza el estado actual
    }
...
/*------------------------------Estado ESPERA: --------------------------------*/   
  ...
  // Una vez se hayan tomado todos los datos de promedio se pregunta si ya se cumplió el tiempo de espera para enviar los datos de promedios
    if (tiempo_transcurrido >= tiempo_muestreo){
      ...
      ESTADO=ENVIO;// Se actualiza el estado actual
    }else{
      ESTADO = TIMER;
    }
  break;
